...
def main(): 
    ...
    # manipulação dos dados do arquivo binario cópia
    # ex: inverter os bytes
    ...
    modifi_data = np.frombuffer(data, dtype=np.uint8).reshape(img_data.shape)
    modified_img = Image.fromarray(modifi_data)
    modified_img.show()
